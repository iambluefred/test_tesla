...
# Steer angle limits
ANGLE_MAX_BP = [0.0, 27.0, 36.0]
ANGLE_MAX_V = [410.0, 92.0, 36.0]

ANGLE_DELTA_BP = [0.0, 5.0, 15.0]
ANGLE_DELTA_V = [5.0, 0.8, 0.25]  # windup limit
ANGLE_DELTA_VU = [5.0, 3.5, 0.8]  # unwind limit
# LDW WARNING LEVELS
LDW_WARNING_1 = 0.9
LDW_WARNING_2 = 0.5
LDW_LANE_PROBAB = 0.2
...
def gen_solution(CS):
    ...
    gps_fix = {
        "bearing": CS.gpsHeading,  # heading of motion in degrees
        ...
        "vNED": [0.0, 0.0, 0.0],  # velocity in NED frame in m/s
        "speedAccuracy": 0.0,  # speed accuracy in m/s
        "verticalAccuracy": 0.0,  # vertical accuracy in meters
        "bearingAccuracy": 0.0,  # heading accuracy in degrees
        "source": "ublox",
...
class CarController:
    def __init__(self, dbc_name, CP, VM):
        ...
        self.ALCA = ALCAController(
            self, True, True
        )  # Enabled and SteerByAngle both True
        ...
        self.roadSignStopDist = 1000.0
        self.roadSignColor = 0.0
        self.roadSignControlActive = 0
        self.roadSignType_last = 0xFF

        self.roadSignDistanceWarning = 50.0
    ...
    def update(
        ...
        if frame % 100 == 0:
            ...
            if CS.hasTeslaIcIntegration:
                ...
                self.set_speed_limit_active = (
                    self.params.get("SpeedLimitOffset") is not None
                ) and (self.params.get("LimitSetSpeed") == "1")
        ...
        if frame % 10 == 0:
            (
                (self.accPitch, self.accRoll, self.accYaw),
                (self.magPitch, self.magRoll, self.magYaw),
                (self.gyroPitch, self.gyroRoll, self.gyroYaw),
            ) = self.GYRO.update(CS.v_ego, CS.a_ego, CS.angle_steers)
            CS.UE.uiGyroInfoEvent(
                self.accPitch,
                self.accRoll,
                self.accYaw,
                self.magPitch,
        ...
        if frame % 10 == 0:
            self.ALCA.update_status(
                ...
                and (
                    (CS.enableALCA and not CS.hasTeslaIcIntegration)
                    or (CS.hasTeslaIcIntegration and CS.alcaEnabled)
                )
            )
        ...
        if self.ACC.enable_adaptive_cruise:
            ...
            CS.v_cruise_pcm = max(0.0, CS.v_ego * CV.MS_TO_KPH) * speed_uom_kph
        self.alca_enabled = self.ALCA.update(
            enabled, CS, actuators, self.alcaStateData, frame, self.blinker
        )
        ...
        enable_steer_control = (
            enabled and not human_lane_changing and not human_control and vehicle_moving
        )

        angle_lim = interp(CS.v_ego, ANGLE_MAX_BP, ANGLE_MAX_V)
        apply_angle = clip(apply_angle, -angle_lim, angle_lim)
        # Windup slower.
        if self.last_angle * apply_angle > 0.0 and abs(apply_angle) > abs(
            self.last_angle
        ):
            ...
            if pathPlanMsg is not None:
                ...
                if self.alcaStateData is not None:
                    self.handlePathPlanSocketForCurvatureOnIC(
                        pathPlanMsg=pathPlanMsg, alcaStateData=self.alcaStateData, CS=CS
                    )
        ...
        if (
            enabled
            and self.ALCA.laneChange_cancelled
            and (not CS.steer_override)
            and (CS.turn_signal_stalk_state == 0)
            and (self.ALCA.laneChange_cancelled_counter > 0)
        ):
        ...
        if (
            ...
            or (self.opState == 2)
            or (self.opState == 5)
            or (self.stopSignWarning != self.stopSignWarning_last)
            or (self.stopLightWarning != self.stopLightWarning_last)
            or (self.warningNeeded == 1)
            or (frame % 100 == 0)
        ):
            # if it's time to send OR we have a warning or emergency disable
            can_sends.append(
                teslacan.create_fake_DAS_warning(
    ...
    def handlePathPlanSocketForCurvatureOnIC(self, pathPlanMsg, alcaStateData, CS):
        ...
        if pp.paramsValid:
            ...
            self.curv0 = -clip(
                pp.dPoly[3], -3.5, 3.5
            )  # self.curv0Matrix.add(-clip(pp.cPoly[3],-3.5,3.5))
            self.curv1 = -clip(
                pp.dPoly[2], -0.2, 0.2
            )  # self.curv1Matrix.add(-clip(pp.cPoly[2],-0.2,0.2))
            self.curv2 = -clip(
            ...
            if alcaStateData.alcaEnabled:
                ...
                self.curv0 = (
                    -self.ALCA.laneChange_direction
                    * alcaStateData.alcaLaneWidth
                    * alcaStateData.alcaStep
                    / alcaStateData.alcaTotalSteps
                )  # animas late change on IC
            ...
            self.lLine = 0
            self.rLine = 0
            self.curv0 = self.curv0Matrix.add(0.0)
            self.curv1 = self.curv1Matrix.add(0.0)
            self.curv2 = self.curv2Matrix.add(0.0)
            self.curv3 = self.curv3Matrix.add(0.0)
            ...
            for tr_ev in tr_ev_list.trafficEvents:
                ...
                if tr_ev.type == 0x04:
                    if tr_ev.distance < self.stopLight_distance:
                        ...
                        self.stopLight_color = (
                            1.0  # 0-unknown, 1-red, 2-yellow, 3-green
                        )
                if tr_ev.type == 0x01:
                    if tr_ev.distance < self.stopLight_distance:
                        self.stopLight_visible = True
                ...
                if tr_ev.type == 0x02:
                    if tr_ev.distance < self.stopLight_distance:
                        ...
                        self.stopLight_resume = tr_ev.resuming
                        self.stopLight_color = (
                            2.0  # 0-unknown, 1-red, 2-yellow, 3-green
                        )
                if tr_ev.type == 0x03:
                    if tr_ev.distance < self.stopLight_distance:
                        self.stopLight_visible = True
                        ...
                        self.stopLight_resume = tr_ev.resuming
                        self.stopLight_color = (
                            3.0  # 0-unknown, 1-red, 2-yellow, 3-green
                        )
            self.checkWhichSign()
            if not (
                (self.roadSignType_last == self.roadSignType)
                and (self.roadSignType == 0xFF)
            ):
                messages.append(
                    teslacan.create_fake_DAS_sign_msg(
                        self.roadSignType,
                        self.roadSignStopDist,
                        self.roadSignColor,
                        self.roadSignControlActive,
                    )
                )
        return messages
    ...
    def _should_ldw(self, CS, frame):
        ...
        return (
            ...
            and frame > self.ldw_numb_frame_end
        )
