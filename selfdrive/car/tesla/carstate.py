...
class CarState(CarStateBase):
    def __init__(self, CP):
        ...
        self.cstm_btns = UIButtons(
            self, "Tesla Model S", "tesla", self.enableShowLogo, self.enableShowCar
        )
    ...
    def get_epas_parser(CP, epascan):
        ...
        return CANParser(
            DBC[CP.carFingerprint]["pt"] + "_epas", signals, checks, epascan
        )
    ...
    def get_pedal_parser(CP, pedalcan):
        ...
        return CANParser(
            DBC[CP.carFingerprint]["pt"] + "_pedal", signals, checks, pedalcan
        )
    ...
    def _convert_to_DAS_fusedSpeedLimit(self, speed_limit_uom, speed_limit_type):
        if speed_limit_uom > 0:
            ...
            return int(
                speed_limit_uom / 5 + 0.5
            )  # sign ID in 5 kph/mph increments (7 shows as 5)
        else:
    ...
    def update_ui_buttons(self, btn_id, btn_status):
        ...
        if (
            (btn_id == 3)
            and (self.cstm_btns.btns[btn_id].btn_status > 0)
            and (self.last_visiond != self.cstm_btns.btns[btn_id].btn_label2)
        ):
    ...
    def update(self, cp, epas_cp, pedal_cp):
        ...
        if self.hasTeslaIcIntegration:
            ...
            self.mapAwareSpeed = (
                cp.vl["MCU_chassisControl"]["MCU_aebEnable"] == 1
                and self.useTeslaMapData
            )
            # AHB info
        ...
        if usu == 1:
            ...
            self.userSpeedLimitOffsetKph = (
                cp.vl["UI_gpsVehicleSpeed"]["UI_userSpeedOffset"] * CV.MPH_TO_KPH
            )
        ...
        if self.baseMapSpeedLimitMPS > 0 and (
            ...
            self.speed_limit_ms = (
                cp.vl["UI_gpsVehicleSpeed"]["UI_mppSpeedLimit"] * map_speed_uom_to_ms
            )
        ...
        self.steer_not_allowed = epas_cp.vl["EPAS_sysStatus"]["EPAS_eacStatus"] not in [
            2,
            1,
        ...
        self.turn_signal_blinking = (
            self.turn_signal_state_left == 1 or self.turn_signal_state_right == 1
        )
