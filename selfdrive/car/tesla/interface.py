...
class CarInterface(CarInterfaceBase):
    ...
    @staticmethod
    def get_params(
        candidate, fingerprint=gen_empty_fingerprint(), has_relay=False, car_fw=[]
    ):
        ...
        mass_models = 4722.0 / 2.205 + STD_CARGO_KG
        ...
        if candidate == CAR.MODELS:
            ...
            if teslaModel == "S":
                ret.longitudinalTuning.kpBP = [0.0, 5.0, 22.0, 35.0]
                ret.longitudinalTuning.kpV = [0.50, 0.45, 0.4, 0.4]
                ret.longitudinalTuning.kiBP = [0.0, 5.0, 22.0, 35.0]
        ...
        ret.minEnableSpeed = (
            -1.0 if (stop_and_go or ret.enableGasInterceptor) else 25.5 * CV.MPH_TO_MS
        )

        centerToRear = ret.wheelbase - ret.centerToFront
        # TODO: get actual value, for now starting with reasonable value for Model S
        ret.rotationalInertia = (
            rotationalInertia_models
            * ret.mass
            * ret.wheelbase ** 2
            / (mass_models * wheelbase_models ** 2)
        )

        # TODO: start from empirically derived lateral slip stiffness and scale by
        # mass and CG position, so all cars will have approximately similar dyn behaviors
        ret.tireStiffnessFront = (
            (tireStiffnessFront_models * ts_factor)
            * ret.mass
            / mass_models
            * (centerToRear / ret.wheelbase)
            / (centerToRear_models / wheelbase_models)
        )
        ret.tireStiffnessRear = (
            (tireStiffnessRear_models * ts_factor)
            * ret.mass
            / mass_models
            * (ret.centerToFront / ret.wheelbase)
            / (centerToFront_models / wheelbase_models)
        )
        ...
        ret.steerMaxBP = [0.0, 15.0]  # m/s
        ret.steerMaxV = [420.0, 420.0]  # max steer allowed

        ret.gasMaxBP = [0.0, 20.0]  # m/s
        ret.gasMaxV = [
            0.225,
            0.525,
        ]  # if ret.enableGasInterceptor else [0.] # max gas allowed
        ret.brakeMaxBP = [0.0]  # m/s
        ret.brakeMaxV = [
            1.0
    ...
    def update(self, c, can_strings):
        ...
        ret.yawRate = self.VM.yaw_rate(
            self.CS.angle_steers * CV.DEG_TO_RAD, self.CS.v_ego
        )
        ...
        ret.brakeLights = bool(
            self.CS.brake_switch or c.actuators.brake > brakelights_threshold
        )
        ...
        if self.CS.prev_turn_signal_stalk_state != self.CS.turn_signal_stalk_state:
            if (
                self.CS.turn_signal_stalk_state == 1
            ...
            if (
                self.CS.turn_signal_stalk_state == 2
        ...
        if self.CP.enableCruise:
            ...
            if (
                (cur_time - self.last_enable_pressed) < 0.2
                and (  # pylint: disable=chained-comparison
                    cur_time - self.last_enable_sent
                )
                > 0.2
