...
def calc_cruise_accel_limits(v_ego, following, is_tesla=False):i
  if is_telsa:
    ...
    if following:
      a_cruise_max = interp(v_ego, _A_CRUISE_MAX_BP, _A_CRUISE_MAX_V_FOLLOWING_TESLA)
    else:
...
def limit_accel_in_turns(v_ego, angle_steers, a_target, CP):
  ...
  """
  if CP.carName == "tesla":
    a_total_max = interp(v_ego, _A_TOTAL_MAX_BP, _A_TOTAL_MAX_V_TESLA)
...
class Planner():
  ...
  def update(self, sm, pm, CP, VM, PP):
    ...
    if enabled and not self.first_loop and not sm['carState'].gasPressed:
      is_tesla = (CP.carName == "tesla")
      accel_limits = [float(x) for x in calc_cruise_accel_limits(v_ego, following, is_tesla)]
